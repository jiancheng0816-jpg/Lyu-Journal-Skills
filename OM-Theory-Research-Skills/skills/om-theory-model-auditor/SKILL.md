...
description: Audit analytical OM and quantitative-management theory models for mathematical, equilibrium, and economic correctness, then triage proposition-level contribution integrity. Implicitly trigger for Chinese/English requests such as 检查模型、检验推导、验证结论、检查证明、命题是否成立、均衡是否正确、找反例、model audit, verify derivation, proof check, validate proposition, check equilibrium, or robustness audit. Do not trigger for prose-only polishing, literature-only searches, journal selection, reviewer-letter drafting, or full-paper writing unless correctness verification is explicitly part of the task.
---
